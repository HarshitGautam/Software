...
f1 = open('mapping/ncro2mirna.txt', 'r')
f2 = open('output/mirna_pubmed.ttl', 'w')
...
for line in f1:
    ...
    while True:
        ...
        # Query url to get pubmed ids for a particular mirna name
        url = 'http://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=pubmed&term=%22' + mirna + '%22&retstart=' + str(retstart) + '&retmax=' + str(retmax) + '&retmode=json'
        try:
            # Send the request to NCBI
            response = urllib.request.urlopen(url)
        except:
            # Upon error, wait 3 seconds and retry
            print('Retrying ' + mirna + ' in 3 seconds.')
            time.sleep(3)
            continue

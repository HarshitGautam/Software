...
$(document).ready(function() {
    ...
    $('#download_btn').on('click', function() {
        ...
        if($('#download_selected_radio').is(':checked')) {
            format = 'txt';
            downloaded = true;
            window.open('http://omnisearch.soc.southalabama.edu:8080/search.php?type=download&mirna=' + mirna + '&term=' + term + '&page=' + page + '&limit=' + limit +
                '&format=' + format + '&selected=' + ($('#select_all_cb').is(':checked') ? 'all' : selected.toString()), '_blank');
        }
        else if($('#download_all_radio').is(':checked')) {
            window.open('http://omnisearch.soc.southalabama.edu:8080/search.php?type=download&mirna=' + mirna + '&term=' + term + '&page=' + page + '&limit=' + limit + '&format=' + format, '_blank');
        }
    });
    ...
    $('#david_btn').on('click', function() {
        ...
        if(downloaded) {
            ...
            if(selected.length === 0) {
                $.get('http://omnisearch.soc.southalabama.edu:8080/search.php', { type: 'all-targets', mirna: mirna, term: term })
                    .done(function(data) {
...
function mirna_query(focus) {
	...
	$.ajax({
		url: 'http://omnisearch.soc.southalabama.edu:8080/query.php',
		data: { mirna: $('#mirna_searchbox_input').val().toLowerCase().trim() },
		type: 'GET',
		dataType: 'jsonp',
		crossDomain: true,
	...
	/*
    $.get('http://omnisearch.soc.southalabama.edu:8080/query.php', { mirna: $('#mirna_searchbox_input').val().toLowerCase().trim() })
        .done(function(data) {
            $('#mirna_searchbox_div').html(data);
            setTimeout(function () {
                if(focus)
...
function term_query(focus) {
    ...
    $.get('http://omnisearch.soc.southalabama.edu:8080/query.php', { term: $('#term_searchbox_input').val().toLowerCase().trim() })
        .done(function(data) {
...
function search() {
    ...
    $.getJSON('http://omnisearch.soc.southalabama.edu:8080/search.php', { type: 'search', mirna: mirna, term: term, page: page, limit: limit })
        .done(function(data) {
            if(data.success) {
                page = data.page;
        ...
        .fail(function() {
			...
			// Will create an error page. (Harrison said that on 11/24/2015.)

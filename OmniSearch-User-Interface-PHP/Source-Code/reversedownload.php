...
try {
    ...
    if($format == 'selected') {
        ...
        // Create the filename
        $filename = 'microRNA_List_for_' . $_GET['gene'] . '-' . date('Y-m-d') . '.txt';
        ...
        if ($format === 'csv') {
            ...
            foreach($json['results']['bindings'] as $item) {
                if($partial && !in_array($item['mirna_label']['value'], $_SESSION['selected'], true))
                    continue;
            ...
            // Create the filename
            $filename = 'Query_Results_for_' . $gene . '-' . date('Y-m-d') . '.' . $format;
        }
        else if ($format === 'tsv') {
            // Write description comment
            // Write description comment
            $text .= "TargetGene\Targeting miRNA\t" .
                ($show_mirdb ? "miRDB_score\t" : "") .
                ($show_targetscan ? "TargetScan_score\t" : "") .
                ($show_miranda ? "miRanda_score\t" : "") .
                ($show_mirtarbase ? "miRTarBase_id\t" : "") .
            ...
            // Create the filename
            $filename = 'Query_Results_for_' . $gene . '-' . date('Y-m-d') . '.txt';
        }

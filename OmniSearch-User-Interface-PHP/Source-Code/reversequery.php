...
try {
	...
	$mesh = $_GET['mesh'];
	 ...
	 if( isset($_GET['gene'])) {
        // If type is gene
        if ($type == 'gene') {
           $query = 'PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#> PREFIX obo: <http://purl.obolibrary.org/obo/> PREFIX xsd: <http://www.w3.org/2001/XMLSchema#> SELECT ?label WHERE { ?child rdfs:subClassOf obo:NCRO_0000025 . ?child rdfs:label ?label FILTER STRSTARTS(UCASE(?label), UCASE("'. $gene .'")) } ORDER BY ?label';
